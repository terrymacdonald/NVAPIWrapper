...
## Project Scope
- Purpose: Safe C# wrapper over NVIDIA NVAPI (vtable COM style) for Windows x64 targeting .NET 10.0 and higher.
- Structure: 
...
- Two API Levels: There are two levels of NVAPIWRapper API available for users to use. 
    - Native: This level is low level and use the C# equivalent functions in NVAPIWrapper that were created by ClangSharpPInvokeGenerator, and are found in `../NVAPIWrapper/cs_generated`. These are the functions that are the C# equvalent of the C++ NVIDIA NVAPI SDK described in the ../NVAPI/SDK/Include files, so please look there if you need to know what they are.
